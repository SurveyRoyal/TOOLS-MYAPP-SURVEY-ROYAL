...
# Light UI (WPF) – PS 5.1 compatible (khong dung toan tu ternary)
# Tabs: Install / CSVV / FONT / AutoCAD
# Tinh nang: "Chay ngoai Console" + "1 Console" (gom winget vao 1 cua so), fix newline dính dòng
...
# Giữ 1 cửa sổ console (tùy chọn), và ghi file với newline chuẩn
...
function Start-ExternalConsoleFromLines([string]$title,[string[]]$lines){
  try{
    ...
    # ⚠️ GHIM NEWLINE RÕ RÀNG (fix dính dòng)
    $scriptText = (($preamble + $lines) -join [Environment]::NewLine)
    Set-Content -Path $tmp -Value $scriptText -Encoding UTF8

    # (Tùy chọn) Đóng console cũ trước khi mở console mới
    if ($global:ExtConsoleProc -and -not $global:ExtConsoleProc.HasExited) {
      try { $global:ExtConsoleProc.CloseMainWindow() | Out-Null; Start-Sleep -Milliseconds 400 } catch {}
...
function Launch-OneConsoleWingetBatch([object[]]$wingetArgsList){
  ...
  foreach($a in $wingetArgsList){
    $json = ($a | ConvertTo-Json -Compress)
    $lines += '$a = @'''+$json+'''@ | ConvertFrom-Json; RunWinget $a'
  }
...
function Install-ZipPackage([hashtable]$zip){
  ...
  if($ChkConsole.IsChecked){
    $lines = @(
      ...
      '  $sc = $ws.CreateShortcut($lnk); $sc.TargetPath = $target;',
      '  if ("' + $args + '" -ne "") { $sc.Arguments = "' + $args + '" }',
      '  $sc.WorkingDirectory = $dest; $sc.Save()',
      '}',
...
# 7-Zip helpers
function Find-7z(){ $c=Get-Command 7z -ErrorAction SilentlyContinue; if($c){return $c.Source}
  foreach($p in @("C:\Program Files\7-Zip\7z.exe","C:\Program Files (x86)\7-Zip\7z.exe")){ if(Test-Path $p){ return $p } }
  return $null }
function Ensure-7Zip(){ $exe=Find-7z; if($exe){return $exe}
...
function Extract-7z([string]$archive,[string]$dest,[string]$password=""){
  $seven = Ensure-7Zip; if($seven){ } else { Log-Msg "[ERR] Khong tim thay 7-Zip (7z.exe)."; return $false }
  if(-not (Test-Path $dest)){ New-Item -ItemType Directory -Path $dest -Force | Out-Null }
  $args = @("x","-y","-aoa","-o$dest",$archive); if($password){ $args = @("x","-y","-aoa","-p$password","-o$dest",$archive) }
  Log-Msg ("7z: {0} {1}" -f $seven, ($args -join ' '))
  $p = Start-Process -FilePath $seven -ArgumentList $args -PassThru -WindowStyle Hidden; $p.WaitForExit()
...
function Extract-ArchiveAny([string]$file,[string]$dest,[string]$password=""){
  ...
  if($ext -eq ".zip"){ try{ Add-Type -AssemblyName System.IO.Compression.FileSystem | Out-Null } catch {}
    [System.IO.Compression.ZipFile]::ExtractToDirectory($file,$dest,$true); return $true }
  elseif($ext -in @(".7z",".rar",".001",".cab")){ return (Extract-7z -archive $file -dest $dest -password $password) }
  else { return $false }
}

# MEGA (tuy chon)
function Ensure-MegaCmd(){
  $cmd = Get-Command mega-get -ErrorAction SilentlyContinue; if($cmd){ return $true }
  foreach($id in @("MEGA.MEGAcmd","MegaLimited.MEGAcmd","MEGA.nz.MEGAcmd")){
    try{
      $p = Start-Process -FilePath "winget" -ArgumentList @("show","-e","--id",$id) -PassThru -WindowStyle Hidden; $p.WaitForExit()
      if($p.ExitCode -eq 0){
...
function Mega-DownloadToTemp([string]$megaUrl){
  ...
  $outDir = Join-Path $env:TEMP ("mega_" + (Get-Random)); New-Item -ItemType Directory -Path $outDir -Force | Out-Null
  Log-Msg ("MEGA: mega-get -> {0}" -f $outDir)
  $p = Start-Process -FilePath "mega-get" -ArgumentList @($megaUrl,$outDir) -PassThru -WindowStyle Hidden; $p.WaitForExit()
  if($p.ExitCode -ne 0){ Log-Msg ("[ERR] mega-get exit {0}" -f $p.ExitCode); return $null }
  $items = Get-ChildItem -Path $outDir -Force -ErrorAction SilentlyContinue; if(-not $items){ return $outDir }
  ($items | Sort-Object LastWriteTime -Descending | Select-Object -First 1).FullName
}

# Bien doi URL cloud -> direct
function Transform-UrlForDownload([string]$url){
  try{
    if($url -match 'onedrive\.live\.com'){
      $u=[Uri]$url; $q=[System.Web.HttpUtility]::ParseQueryString($u.Query)
      if($q["cid"] -and $q["resid"]){ $auth = if($q["authkey"]){ "&authkey=$($q['authkey'])" } else { "" }; return "https://onedrive.live.com/download?cid=$($q['cid'])&resid=$($q['resid'])$auth" }
    }
    if($url -match 'sharepoint\.com'){ if($url -match '\?'){ if($url -notmatch 'download=1'){ return "$url&download=1" } } else { return "$url?download=1" } }
    if($url -match 'dropbox\.com'){ if($url -match 'dl=0'){ return ($url -replace 'dl=0','dl=1') } else { return "$url`?dl=1" } }
  } catch {}
  return $url
}
function Get-LocalFromSource([string]$src,[string]$password=""){
  ...
  if($src -match '^https?://'){
    if($src -match 'mega(\.co)?\.nz'){
      $p = Mega-DownloadToTemp $src; if(-not $p){ return $null }
    ...
    } else { return @{ Kind="file"; Path=$tmp } }
  }
...
# AutoCAD
function Invoke-Proc($file,$args,$wd){ $p = Start-Process -FilePath $file -ArgumentList $args -WorkingDirectory $wd -PassThru -WindowStyle Hidden; $p.WaitForExit(); $p.ExitCode }
function Install-AutoCADFromExe([string]$file){
  ...
  if($file.ToLower().EndsWith(".msi")){
    $msiArgs = "/i `"$file`" /qn /norestart"; Log-Msg ("MSI: msiexec {0}" -f $msiArgs)
    $ec = Invoke-Proc "msiexec.exe" $msiArgs $wd; if($ec -eq 0){ Log-Msg "[OK] MSI installed"; return $true } else { Log-Msg ("[WARN] MSI exit {0}" -f $ec); return $false }
  }
  $cands = @("/quiet","/silent","--quiet","--silent","/S","/VERYSILENT","/s /v`"/qn REBOOT=ReallySuppress`"","-q","")
  foreach($a in $cands){ Log-Msg ("Try: `"{0}`" {1}" -f $file,$a); $ec = Invoke-Proc $file $a $wd; if($ec -eq 0){ Log-Msg "[OK] installed"; return $true } }
...
function Install-AutoCADFromFolder([string]$dir){
  ...
  $exe = Get-ChildItem -Path $dir -Recurse -Include AutodeskInstaller.exe,Install.exe,install.exe,Setup.exe,setup.exe,*.exe -ErrorAction SilentlyContinue | Sort-Object Length | Select-Object -First 1
...
function Install-AutoCADAuto([string]$version){
  ...
  $loc = Get-LocalFromSource -src $src -password $pwd; if(-not $loc){ return $false }
  if($loc.Kind -eq "file"){ Log-Msg ("{0}: cai tu EXE/MSI {1}" -f $version,$loc.Path); return Install-AutoCADFromExe $loc.Path }
  Log-Msg ("{0}: cai tu thu muc {1}" -f $version,$loc.Path); return Install-AutoCADFromFolder $loc.Path
}
function Add-AutoCADContextMenu($cb,[string]$ver){
  ...
  $miUrl.Add_Click({ $def = if($AutoCADSources.ContainsKey($ver)){$AutoCADSources[$ver]} else {""}
  ...
  $mi4.Add_Click({ if($AutoCADSources.ContainsKey($ver)){ Log-Msg ("[{0}] Source = {1}" -f $ver,$AutoCADSources[$ver]) }
...
$AppCatalog = @{
  ...
  # HHMap 2019 (EXE)
  "HHMap2019"     = @{ Name="HHMap 2019";      Exe=@{ Url="https://hhmaps.vn/uploads/download/files/hhmaps2019_setup.exe"; Args="/S"; Sha256="" } }
...
# Input helpers
function Ask([string]$title,[string]$label,[string]$def=""){ [Microsoft.VisualBasic.Interaction]::InputBox($label,$title,$def) }
function Pick-File([string]$filter="Executable/Archive (*.exe;*.msi;*.zip;*.7z;*.rar;*.001)|*.exe;*.msi;*.zip;*.7z;*.rar;*.001|All files (*.*)|*.*"){
  $dlg = New-Object System.Windows.Forms.OpenFileDialog; $dlg.Filter = $filter; $dlg.Multiselect=$false
  if($dlg.ShowDialog() -eq 'OK'){ return $dlg.FileName } else { return $null } }
function Pick-Folder(){ $dlg = New-Object System.Windows.Forms.FolderBrowserDialog; if($dlg.ShowDialog() -eq 'OK'){ return $dlg.SelectedPath } else { return $null } }
...
# ===== UI: AutoCAD tab =====
$gbAC = New-Object System.Windows.Controls.GroupBox; $gbAC.Header = "AutoCAD Versions"; $gbAC.Margin = "0,0,0,10"
$panelAC = New-Object System.Windows.Controls.WrapPanel
foreach($ver in $AutoCADVersions){
  $cb = New-Object System.Windows.Controls.CheckBox
  $cb.Style = $window.Resources["TileCheckBox"]; $cb.Content = $ver; $cb.Tag = $ver; $cb.Width = 180; $cb.Height = 38
  $panelAC.Children.Add($cb) | Out-Null; $CheckBoxes[$ver] = $cb
  Add-AutoCADContextMenu -cb $cb -ver $ver
  $cb.AddHandler([System.Windows.Controls.Control]::MouseDoubleClickEvent,
    [System.Windows.Input.MouseButtonEventHandler]{ param($s,$e) $v = $s.Tag; $s.IsEnabled = $false; try{ [void](Install-AutoCADAuto -version $v) } finally { $s.IsEnabled = $true } })
}
$gbAC.Content = $panelAC; $PanelAutoCAD.Children.Add($gbAC) | Out-Null
...
$BtnInstallSelected.Add_Click({
  ...
  foreach($k in $selected){
    ...
    try {
      if($k -like "AutoCAD *"){ [void](Install-AutoCADAuto -version $k); continue }
